...
class OralExplainer(ExplainerScene):
    ...
    def evidence_check(self, result: str, question: str, answer: str, color: str) -> VGroup:
        ...
        result_card = self.make_card("authors report", result, color, 10.8, 1.38)
        question_card = self.make_card("pause and predict", question, ORANGE, 10.8, 1.38)
...
class ICLRTransformerExplainer(OralExplainer):
    def construct(self) -> None:
        ...
        head = self.chapter("5", "Evidence, then a comprehension check", "Do not confuse an authors' report with this toy animation.")
        check = self.evidence_check(
            ...
            "No — zero weight means zero contribution.",
            BLUE,
        )
...
class ICLRDPOExplainer(OralExplainer):
    def construct(self) -> None:
        ...
        head = self.chapter("5", "Evidence, then a comprehension check", "The paper result and the local mechanism answer different questions.")
        check = self.evidence_check(
            ...
            "It stays fixed — DPO uses their difference.",
            GREEN,
        )
        self.play(FadeIn(head), FadeIn(check[0], shift=UP * 0.08))
        self.play(FadeIn(check[1], shift=UP * 0.08))
...
class ICLRESExplainer(OralExplainer):
    def construct(self) -> None:
        ...
        head = self.chapter("5", "Evidence, then a comprehension check", "Batch reliability is not the same quantity as reward.")
        check = self.evidence_check(
            ...
            "It falls — the evidence is concentrated.",
            ORANGE,
        )
        self.play(FadeIn(head), FadeIn(check[0], shift=UP * 0.08))
        self.play(FadeIn(check[1], shift=UP * 0.08))
...
class ICLRRRoPEExplainer(OralExplainer):
    def construct(self) -> None:
        ...
        head = self.chapter("5", "Evidence, then a comprehension check", "The geometric identity motivates the empirical comparison.")
        check = self.evidence_check(
