(() => {
  ...
  const defaultStatusText = () => {
    const stages = currentRun()?.trace_summary?.stage_count ?? "unrecorded";
    return `${state.catalog.runs.length} rendered model run${state.catalog.runs.length === 1 ? "" : "s"} · ${state.catalog.papers.length} sourced paper${state.catalog.papers.length === 1 ? "" : "s"} · ${stages} recorded JSON API stages · deterministic renderer`;
  };
  ...
  function renderOverview() {
    ...
    const paperScope = state.catalog.papers.map((paper) => paper.short_title).join(" + ");
    const includesReplay = state.catalog.runs.some((item) => item.generation_modes.includes("frozen_replay"));
    const ledgerNote = `Totals cover ${paperScope}. Live costs use the frozen provider rate card.${includesReplay ? " Reviewed replays have no recorded API usage." : ""}`;
    ...
    $("#overview-view").innerHTML = `
      ...
      <section class="quality-audit" aria-labelledby="quality-audit-title">
        ...
        <dl><div><dt>Selected narrative</dt><dd>${isLive ? "Generated by the selected model API; no manual editing after generation" : "Human/Codex-reviewed from paper, code, and iterative reader feedback"}</dd></div><div><dt>Live model API</dt><dd>${liveRuns.length ? `${liveRuns.length} complete run${liveRuns.length === 1 ? "" : "s"}, each covering ${state.catalog.papers.length} sourced paper${state.catalog.papers.length === 1 ? "" : "s"} under the same contract` : "No complete live run is published yet"}</dd></div><div><dt>Harness</dt><dd>Validation, deterministic maps, registered Manim rendering when selected, traces, and publication</dd></div></dl>
        <p>${isLive ? "Judge its section split, narrative continuity, evidence use, and animation decisions—not just whether its JSON passed." : "Use the model switcher to compare section boundaries, block balance, and Manim choices without blending authorship."}</p>
      </section>
  ...
  function renderBacktranslation() {
    ...
    const publication = model.publication || { status: "withheld", verified_distinct_videos: 0, minimum_required: 2 };
    const publicationEvidence = publication.evidence_url
      ? `<a href="${esc(publication.evidence_url)}">Publication evidence ↗</a>`
      : "";
    $("#backtranslation-view").innerHTML = `
      ...
      <section class="bt-candidate-bar" aria-label="Backtranslation model candidate">
        <div><span class="eyebrow">Candidate</span><strong>${esc(model.label)}</strong><small>${esc(model.provider)} · ${esc(model.model_id)}</small><small>${esc(model.status.replaceAll("_", " "))}</small><small>${adapterEvidence}</small><small>${publication.status === "published" ? `${publication.verified_distinct_videos} distinct local videos · publication gate passed` : `${publication.verified_distinct_videos || 0}/${publication.minimum_required || 2} verified videos · candidate withheld`}</small>${publicationEvidence}</div>
